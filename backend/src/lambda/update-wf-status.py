...
def lambda_handler(event, context):
    ...
    try:
        ...
        update = f"UPDATE \"{env_vars['PII_TABLE']}\" SET docs=? SET status=? set phi_input=? WHERE part_key=? AND sort_key=? RETURNING ALL NEW *"
        ...
        redact = deserialized_document['redact']
    ...
    return dict(workflow_id=workflow_id, bucket=bucket, phi_input_dir=phi_input_dir, redact=redact)

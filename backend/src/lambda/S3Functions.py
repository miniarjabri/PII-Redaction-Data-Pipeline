...
This module provides a helper class for interacting with Amazon S3.
It simplifies common S3 operations used throughout the Lambda functions
in the PII redaction pipeline, such as:
...
The class provides a consistent interface for S3 operations and handles
...
class S3:
    ...
    This class provides methods for common S3 operations used in the PII redaction pipeline.
    It simplifies the interaction with S3 and provides consistent error handling.
    """
    def __init__(self, bucket: str, log_level: str ='INFO'):
        """
        Initialize the S3 helper with a bucket name and log level.
    ...
    def list_objects(self, prefix: str, filters: list = None, search: list = None) -> list:
        ...
        Optionally filter objects by including or excluding patterns.
        
        Args:
            prefix: S3 prefix to list objects from
            filters: List of patterns to exclude from results
            search: List of patterns to include in results
            
        Returns:
            list: List of object keys matching the criteria
        """
        try:
    ...
    def list_prefixes(self, prefix: str) -> list:
        """
        List directory-like prefixes in the S3 bucket with the given prefix.
        
        Args:
            prefix: S3 prefix to list directories from
            
        Returns:
            list: List of directory prefixes
        """
        try:
    ...
    def copy_object(self, source_object: str, destination_object: str) -> bool:
        ...
        Args:
            source_object: Source S3 object key
            destination_object: Destination S3 object key
    ...
    def copy_objects(self, source_prefix: str, destination_prefix: str, filters: list = None, search: list = None) -> bool:
        """
        Copy multiple objects with the same prefix to a new location.
        
        Args:
            source_prefix: Source S3 prefix
            destination_prefix: Destination S3 prefix
            filters: List of patterns to exclude from copying
            search: List of patterns to include in copying
            
        Returns:
            bool: True if all objects were copied successfully
        """
        try:
    ...
    def move_object(self, source_object: str, destination_object: str) -> dict:
        """
        Move an object within the same S3 bucket (copy and delete).
    ...
    def move_objects(self, source_prefix: str, destination_prefix: str, filters: list=None, search: list = None) -> bool:
        """
        Move multiple objects with the same prefix to a new location.
        
        Args:
            source_prefix: Source S3 prefix
            destination_prefix: Destination S3 prefix
            filters: List of patterns to exclude from moving
            search: List of patterns to include in moving
            
        Returns:
            bool: True if all objects were moved successfully
        """
        try:
    ...
    def delete_objects(self, objects: list) -> dict:
        ...
        Args:
            objects: List of S3 object keys to delete
    ...
    def delete_prefix(self, prefix: str, filters: list = None) -> dict:
        """
        Delete all objects with the given prefix from the S3 bucket.
        
        Args:
            prefix: S3 prefix to delete objects from
            filters: List of patterns to exclude from deletion
    ...
    def upload_file(self, source_file: str, destination_object: str, ExtraArgs: dict = None) -> bool:
        ...
        Args:
            ...
            destination_object: S3 object key
            ExtraArgs: Extra arguments for the upload (e.g., ContentType)
